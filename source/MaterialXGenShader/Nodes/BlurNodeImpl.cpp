...
string BlurNodeImpl::BOX_FILTER = "box";
string BlurNodeImpl::GAUSSIAN_FILTER = "gaussian";
string BlurNodeImpl::BOX_WEIGHT_FUNCTION = "sx_get_box_weights";
string BlurNodeImpl::GAUSSIAN_WEIGHT_FUNCTION = "sx_get_gaussian_weights";

BlurNodeImpl::BlurNodeImpl()
    : ParentClass()
...
ShaderNodeImplPtr BlurNodeImpl::create()
{
    return std::shared_ptr<BlurNodeImpl>(new BlurNodeImpl());
}

void BlurNodeImpl::computeSampleOffsetStrings(const string& sampleSizeName, const string& offsetTypeString, StringVec& offsetStrings)
{
    offsetStrings.clear();
...
bool BlurNodeImpl::acceptsInputType(const TypeDesc* type)
{
...
void BlurNodeImpl::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderGenerator& shadergen, Shader& shader)
{
    ...
    {
        throw ExceptionShaderGenError("Node '" + node.getName() + "' is not a valid BlurNodeImpl node");
    }
