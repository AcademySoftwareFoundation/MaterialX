...
void ShaderGraph::addInputSockets(const InterfaceElement& elem, GenContext& context)
{
    for (InputPtr input : elem.getActiveInputs())
    {
        ...
        const TypeDesc portType = context.getShaderGenerator().getTypeSystem()->getType(input->getType());
...
void ShaderGraph::addOutputSockets(const InterfaceElement& elem, GenContext& context)
{
    ...
    {
        addOutputSocket(output->getName(), context.getShaderGenerator().getTypeSystem()->getType(output->getType()));
    }
    if (numOutputSockets() == 0)
    {
        addOutputSocket("out", context.getShaderGenerator().getTypeSystem()->getType(elem.getType()));
    }
}
    ...
    if (!node)
    {
        ...
        if (!space.empty())
        {
            ...
            if (spaceInput && nodeDefSpaceInput)
            {
                ...
                const TypeDesc portType = context.getShaderGenerator().getTypeSystem()->getType(nodeDefSpaceInput->getType());
...
ShaderGraphPtr ShaderGraph::create(const ShaderGraph* parent, const string& name, ElementPtr element, GenContext& context)
{
    ...
    if (element->isA<Output>())
    {
        ...
        const TypeDesc outputType = context.getShaderGenerator().getTypeSystem()->getType(output->getType());
