...
TEST_CASE("GenShader: Surface Material In Nodegraph With Interface Input", "[genshader]")
{
    // Regression test: a <surfacematerial> placed inside a <nodegraph> whose surface shader
    // input is bound through the graph interface (interfacename) to an external node. When
    // generating a shader for this graph, the surface shader connection resolves to the
    // ShaderGraph's own input socket, whose owning node has no implementation. This used to
    // dereference a null shared_ptr in ShaderNode::getImplementation(); it must now fail
    // gracefully with a MaterialX exception (or generate cleanly) rather than crash.
    // The nodegraph interface input has NO upstream node: the traversal therefore stops at the
    // interface boundary and never overwrites the ShaderGraph-socket connection that
    // createNode() establishes for the interfacename binding. That leaves mat's surfaceshader
    // input pointing at the ShaderGraph's own input socket, whose owning node (_impl==null)
    // is then passed to emitFunctionCall, triggering the null dereference.
    const std::string testDocumentString =
        "<?xml version=\"1.0\"?>"
        "<materialx version=\"1.39\">"
        "  <nodegraph name=\"green_material_graph\">"
        "    <input name=\"input_shader\" type=\"surfaceshader\" />"
        "    <surfacematerial name=\"mat\" type=\"material\">"
        "      <input name=\"surfaceshader\" type=\"surfaceshader\" interfacename=\"input_shader\" />"
        "    </surfacematerial>"
        "    <output name=\"out\" type=\"material\" nodename=\"mat\" />"
        "  </nodegraph>"
        "</materialx>";
...
#ifdef MATERIALX_BUILD_GEN_GLSL
    {
        ...
        {
            // Must not crash; either succeeds or throws a MaterialX exception.
            try
            {
