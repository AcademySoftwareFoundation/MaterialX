...
void bindPyOslRenderer(py::module& mod)
{
    py::class_<mx::OslRenderer, mx::ShaderRenderer, mx::OslRendererPtr>(mod, "OslRenderer", "Helper class for rendering generated OSL code to produce images.\n\nThe main services provided are: Source code validation: Use of \"oslc\" to compile and test output results Introspection check: None at this time. Binding: None at this time. Render validation: Use of \"testrender\" to output rendered images. Assumes source compilation was success as it depends on the existence of corresponding .oso files.")
        .def_static("create", &mx::OslRenderer::create, "Create an OSL renderer instance.")
        .def_readwrite_static("OSL_CLOSURE_COLOR_STRING", &mx::OslRenderer::OSL_CLOSURE_COLOR_STRING)
        .def("initialize", &mx::OslRenderer::initialize, py::arg("renderContextHandle") = nullptr, "Internal initialization required for program validation and rendering.\n\nAn exception is thrown on failure. The exception will contain a list of initialization errors.")
        .def("createProgram", static_cast<void (mx::OslRenderer::*)(const mx::ShaderPtr)>(&mx::OslRenderer::createProgram), "Create OSL program based on shader stage source code.\n\nArgs:\n    stages: Map of name and source code for the shader stages.")
        .def("createProgram", static_cast<void (mx::OslRenderer::*)(const mx::OslRenderer::StageMap&)>(&mx::OslRenderer::createProgram), "Create OSL program based on shader stage source code.\n\nArgs:\n    stages: Map of name and source code for the shader stages.")
        .def("validateInputs", &mx::OslRenderer::validateInputs, "Validate inputs for the compiled OSL program.\n\nNote: Currently no validation has been implemented.")
        .def("render", &mx::OslRenderer::render, "Render OSL program to disk.\n\nThis is done by using either \"testshade\" or \"testrender\". Currently only \"testshade\" is supported.\n\nUsage of both executables requires compiled source (.oso) files as input. A shader output must be set before running this test via the setOslOutputName() method to ensure that the appropriate .oso files can be located.")
        .def("captureImage", &mx::OslRenderer::captureImage, "Capture the current rendered output as an image.")
