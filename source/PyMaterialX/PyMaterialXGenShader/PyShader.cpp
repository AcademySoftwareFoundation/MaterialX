...
void bindPyShader(py::module& mod)
{
    ...
    py::class_<mx::Shader, mx::ShaderPtr>(mod, "Shader", "Class containing all data needed during shader generation.\n\nAfter generation is completed it will contain the resulting source code emitted by shader generators.\n\nThe class contains a default implementation using a single shader stage. Derived shaders can override this, as well as overriding all methods that add code to the shader.")
        ...
        .def("getName", &mx::Shader::getName, "Return the shader name.")
        ...
        .def("hasAttribute", &mx::Shader::hasAttribute, "Return true if the shader has a given named attribute.")
        .def("getAttribute", &mx::Shader::getAttribute, "Return the value for a named attribute, or nullptr if no such attribute is found.")
        .def("setAttribute", static_cast<void (mx::Shader::*)(const std::string&)>(&mx::Shader::setAttribute), "Set a flag attribute on the shader.")
        .def("setAttribute", static_cast<void (mx::Shader::*)(const std::string&, mx::ValuePtr)>(&mx::Shader::setAttribute), "Set a flag attribute on the shader.");
}
