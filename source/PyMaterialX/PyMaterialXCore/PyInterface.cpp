...
void bindPyInterface(py::module& mod)
{
    py::class_<mx::PortElement, mx::PortElementPtr, mx::ValueElement>(mod, "PortElement", "The base class for port elements such as Input and Output.\n\nPort elements support spatially-varying upstream connections to nodes.")
        ...
        .def("setConnectedNode", &mx::PortElement::setConnectedNode, "Set the node to which this element is connected.\n\nThe given node must belong to the same node graph. If the node argument is null, then any existing node connection will be cleared.")
        .def("getConnectedNode", &mx::PortElement::getConnectedNode, "Return the node, if any, to which this element is connected.")
        .def("setConnectedOutput", &mx::PortElement::setConnectedOutput, "Set the output to which this input is connected.\n\nIf the output argument is null, then any existing output connection will be cleared.")
        .def("getConnectedOutput", &mx::PortElement::getConnectedOutput, "Return the output, if any, to which this input is connected.");
    ...
    py::class_<mx::InterfaceElement, mx::InterfaceElementPtr, mx::TypedElement>(mod, "InterfaceElement", "The base class for interface elements such as Node, NodeDef, and NodeGraph.\n\nAn InterfaceElement supports a set of Input and Output elements, with an API for setting their values.")
        ...
        .def("getTarget", &mx::InterfaceElement::getTarget, "Return the target string of this interface.")
        ...
        .def("getDeclaration", &mx::InterfaceElement::getDeclaration, py::arg("target") = mx::EMPTY_STRING, "Return the first declaration of this interface, optionally filtered by the given target name.\n\nArgs:\n    target: An optional target name, which will be used to filter the declarations that are considered.\n\nReturns:\n    A shared pointer to declaration, or an empty shared pointer if no declaration was found.")
        .def("clearContent", &mx::InterfaceElement::clearContent, "Clear all attributes and descendants from this element.")
        .def("hasExactInputMatch", &mx::InterfaceElement::hasExactInputMatch, py::arg("declaration"), py::arg("message") = nullptr, "Return true if this instance has an exact input match with the given declaration, where each input of this the instance corresponds to a declaration input of the same name and type.\n\nIf an exact input match is not found, and the optional message argument is provided, then an error message will be appended to the given string.")
