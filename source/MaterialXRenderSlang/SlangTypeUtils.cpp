...
static const std::string DEFAULTVALUE_NAME = "DefaultValue";

std::string getDefaultValueAttribute(slang::VariableReflection* variable, slang::IGlobalSession* globalSession)
{
    slang::Attribute* attr = nullptr;
    if (globalSession)
    {
        attr = variable->findUserAttributeByName(globalSession, "DefaultValue");
    }
    else
    {
        for (uint32_t i = 0; i < variable->getUserAttributeCount(); ++i)
        {
            if (variable->getUserAttributeByIndex(i)->getName() == DEFAULTVALUE_NAME)
            {
                attr = variable->getUserAttributeByIndex(i);
                break;
            }
        }
    }
    ...
    if (attr->getArgumentCount() != 1 || !isSlangString(attr->getArgumentType(0)))
        throw ExceptionRenderError("User attribute `DefaultValue` was found, but has an unexpected format.");
...
template <typename T>
ConstValuePtr makeTypedArrayValue(const std::string& defaultValue, size_t expectedByteSize)
{
    auto result = TypedValue<std::vector<T>>::createFromString(defaultValue);
    auto& vec = result->asA<std::vector<T>>();
    size_t realByteSize = vec.size() * sizeof(T);
    if (realByteSize != expectedByteSize)
        throw ExceptionRenderError("Mismatch between the expected (" + std::to_string(expectedByteSize) + "B) and real (" + std::to_string(realByteSize) + "B) type size.");
    return result;
}

template <typename T>
ConstValuePtr makeTypedValue(const std::string& defaultValue, size_t expectedByteSize)
{
    if (sizeof(T) != expectedByteSize)
        throw ExceptionRenderError("Mismatch between the expected (" + std::to_string(expectedByteSize) + "B) and real (" + std::to_string(sizeof(T)) + "B) type size.");
    return TypedValue<T>::createFromString(defaultValue);
}

template <>
ConstValuePtr makeTypedValue<bool>(const std::string& defaultValue, size_t /*expectedByteSize*/)
{
    /// Slang bool size depends on the target language, so we skip the size check
    /// and on write convert the bool to an int of appropriate size before writing it.
    return TypedValue<bool>::createFromString(defaultValue);
}

template <>
ConstValuePtr makeTypedValue<Matrix33>(const std::string& defaultValue, size_t /*expectedByteSize*/)
{
    /// Slang float3x3 size depends on the target language, so we skip the size check
    /// and on write make sure the memory is properly aligned and padded.
    return TypedValue<Matrix33>::createFromString(defaultValue);
}

template <>
ConstValuePtr makeTypedValue<std::vector<float>>(const std::string& defaultValue, size_t expectedByteSize)
{
    return makeTypedArrayValue<float>(defaultValue, expectedByteSize);
}

template <>
ConstValuePtr makeTypedValue<std::vector<int>>(const std::string& defaultValue, size_t expectedByteSize)
{
    return makeTypedArrayValue<int>(defaultValue, expectedByteSize);
}
...
ConstValuePtr makeDefaultValue(slang::TypeLayoutReflection* typeLayout, const std::string& defaultValue)
{
    ...
    const size_t slangByteSize = getByteSize(typeLayout);
    ...
    if (type->getKind() == slang::TypeReflection::Kind::Scalar)
    {
        ...
        {
            case slang::TypeReflection::ScalarType::Int32:
                return makeTypedValue<int>(defaultValue, slangByteSize);
            case slang::TypeReflection::ScalarType::Float32:
                return makeTypedValue<float>(defaultValue, slangByteSize);
            case slang::TypeReflection::ScalarType::Bool:
                return makeTypedValue<bool>(defaultValue, slangByteSize);
        };
    }
    else if (type->getKind() == slang::TypeReflection::Kind::Vector)
    {
        if (type->getElementType()->getKind() == slang::TypeReflection::Kind::Scalar &&
            type->getElementType()->getScalarType() == slang::TypeReflection::ScalarType::Float32)
        {
            switch (type->getColumnCount())
            {
                case 2:
                    return makeTypedValue<Vector2>(defaultValue, slangByteSize);
                case 3:
                    return makeTypedValue<Vector3>(defaultValue, slangByteSize);
                case 4:
                    return makeTypedValue<Vector4>(defaultValue, slangByteSize);
            }
        }
    }
    else if (type->getKind() == slang::TypeReflection::Kind::Matrix)
    {
        if (type->getElementType()->getKind() == slang::TypeReflection::Kind::Scalar &&
            type->getElementType()->getScalarType() == slang::TypeReflection::ScalarType::Float32)
        {
            if (type->getColumnCount() == 3 && type->getRowCount() == 3)
                return makeTypedValue<Matrix33>(defaultValue, slangByteSize);
            if (type->getColumnCount() == 4 && type->getRowCount() == 4)
                return makeTypedValue<Matrix44>(defaultValue, slangByteSize);
        }
    }
    else if (type->getKind() == slang::TypeReflection::Kind::Array)
    {
        if (type->getElementType()->getKind() == slang::TypeReflection::Kind::Scalar)
        {
            if (type->getElementType()->getScalarType() == slang::TypeReflection::ScalarType::Float32)
                return makeTypedValue<std::vector<float>>(defaultValue, slangByteSize);
            if (type->getElementType()->getScalarType() == slang::TypeReflection::ScalarType::Int32)
                return makeTypedValue<std::vector<int>>(defaultValue, slangByteSize);
        }
    }
    else if (isSamplerTexture2D(type))
    {
        return TypedValue<std::string>::createFromString(defaultValue);
    }

    throw ExceptionRenderError("Cannot parse default value `" + defaultValue + "` for slang type " + getFullName(type) + ".");
}
