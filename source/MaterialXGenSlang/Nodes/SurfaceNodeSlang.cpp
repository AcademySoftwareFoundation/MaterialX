...
void SurfaceNodeSlang::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderStage& stage) const
{
    ...
    DEFINE_SHADER_STAGE(stage, Stage::VERTEX)
    {
        ...
        {
            normal->setEmitted();
            shadergen.emitLine(prefix + normal->getVariable() + " = normalize(mul(float4(" + HW::T_IN_NORMAL + ", 0), " + HW::T_WORLD_INVERSE_TRANSPOSE_MATRIX + ").xyz)", stage);
        }
    ...
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        if (const ShaderNode* bsdf = bsdfInput->getConnectedSibling())
        {
            ...
            {
                shadergen.emitLine("float3 shadowCoord = mul(float4(" + prefix + HW::T_POSITION_WORLD + ", 1.0), " + HW::T_SHADOW_MATRIX + ").xyz", stage);
                ...
                shadergen.emitLine("occlusion = mx_variance_shadow_occlusion(shadowMoments, shadowCoord.z * 2 - 1)", stage);
            }
            shadergen.emitLineBreak(stage);
