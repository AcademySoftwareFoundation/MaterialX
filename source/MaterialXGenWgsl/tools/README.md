...
[`skip_transpile.txt`](skip_transpile.txt) (stdlib `mx_math_platform` / `mx_hextile`, the
sampler-bound pbrlib environment helpers, and the texture/light node shaders naga can't express). Every
other `genwgsl/*.wgsl` is a **derived artifact**: not committed, regenerated by CI before
...
* **Hand-written `.wgsl`** — [`skip_transpile.txt`](skip_transpile.txt) lists every hand-maintained
  genwgsl file (stdlib `mx_math_platform` / `mx_hextile`, the sampler-bound pbrlib `lib/` helpers
  naga can't express, and the texture/light node shaders). The lib pass leaves any `lib/*.wgsl`
  listed there untouched, and `--clean` keeps exactly this set while deleting everything else.
