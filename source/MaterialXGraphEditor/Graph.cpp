...
void Graph::loadStandardLibraries() {
    ...
    try {
        ...
        if (_xincludeFiles.empty()) {
            std::cerr << "Could not find standard data libraries on the given search path: " << _searchPath.asString()
                      << std::endl;
        }
    }
...
ed::PinId Graph::getOutputPin(UiNodePtr node, UiNodePtr upNode, UiPinPtr input) {
    if (upNode->getNodeGraph() != nullptr) {
        ...
        mx::OutputPtr output = input->_pinNode->getNode() ? input->_pinNode->getNode()->getConnectedOutput(input->_name)
                                                          : nullptr;
        if (output) {
                ...
                if (pins.size() != 0 && layoutNode->getInput() == nullptr) {
                    for (size_t i = 0; i < pins.size(); i++) {
                        ...
                        if (nextNode) {
                            ...
                            layoutPosition(nextNode, ImVec2(newPos.x, startingPos.y), initialLayout,
                                           layoutNode->_level + 1);
                        }
                    }
                }
...
void Graph::setUiNodeInfo(UiNodePtr node, const std::string &type, const std::string &category) {
    ...
    if (node->getNodeGraph()) {
        ...
        for (mx::OutputPtr out: outputs) {
            UiPinPtr outPin = std::make_shared<UiPin>(_graphTotalSize, &*out->getName().begin(), out->getType(), node,
                                                      ax::NodeEditor::PinKind::Output, nullptr, nullptr);
            ++_graphTotalSize;
        ...
        for (mx::InputPtr input: node->getNodeGraph()->getInputs()) {
            UiPinPtr inPin = std::make_shared<UiPin>(_graphTotalSize, &*input->getName().begin(), input->getType(),
                                                     node, ax::NodeEditor::PinKind::Input, input, nullptr);
        ...
        if (node->getNode()) {
            ...
            if (nodeDef) {
                for (mx::InputPtr input: nodeDef->getActiveInputs()) {
                    ...
                    UiPinPtr inPin = std::make_shared<UiPin>(_graphTotalSize, &*input->getName().begin(),
                                                             input->getType(), node, ax::NodeEditor::PinKind::Input,
                                                             input, nullptr);
                    node->inputPins.push_back(inPin);
                    _currPins.push_back(inPin);
                    ++_graphTotalSize;
                ...
                for (mx::OutputPtr output: nodeDef->getActiveOutputs()) {
                    ...
                    UiPinPtr outPin = std::make_shared<UiPin>(_graphTotalSize, &*output->getName().begin(),
                                                              output->getType(),
            ...
            UiPinPtr inPin = std::make_shared<UiPin>(_graphTotalSize, &*("Value"), node->getInput()->getType(), node,
                                                     ax::NodeEditor::PinKind::Input, node->getInput(), nullptr);
            node->inputPins.push_back(inPin);
            _currPins.push_back(inPin);
            ++_graphTotalSize;
        } else if (node->getOutput()) {
            UiPinPtr inPin = std::make_shared<UiPin>(_graphTotalSize, &*("input"), node->getOutput()->getType(), node,
                                                     ax::NodeEditor::PinKind::Input, nullptr, node->getOutput());
            node->inputPins.push_back(inPin);
            _currPins.push_back(inPin);
            ++_graphTotalSize;
        }
        ...
        if (node->getInput() || node->getOutput()) {
            UiPinPtr outPin = std::make_shared<UiPin>(_graphTotalSize, &*("output"), type, node,
                                                      ax::NodeEditor::PinKind::Output, nullptr, nullptr);
            ++_graphTotalSize;
            node->outputPins.push_back(outPin);
            _currPins.push_back(outPin);
        ...
        for (UiPinPtr pin: origNode->inputPins) {
            if (origNode->getConnectedNode(pin->_name) && !_ctrlClick) {
                ...
                if (_copiedNodes.find(origNode->getConnectedNode(pin->_name)) != _copiedNodes.end()) {
                    ...
                    // create an edge
                    createEdge(_copiedNodes[origNode->getConnectedNode(pin->_name)], copyNode,
...
void Graph::addNode(const std::string &category, const std::string &name, const std::string &type) {
    ...
    if (node) {
        ...
        for (mx::InputPtr input: defInputs) {
            UiPinPtr inPin = std::make_shared<UiPin>(_graphTotalSize, &*input->getName().begin(), input->getType(),
                                                     newNode, ax::NodeEditor::PinKind::Input, input, nullptr);
        ...
        for (mx::OutputPtr output: defOutputs) {
            UiPinPtr outPin = std::make_shared<UiPin>(_graphTotalSize, &*output->getName().begin(), output->getType(),
                                                      newNode, ax::NodeEditor::PinKind::Output, nullptr, nullptr);
...
UiPinPtr Graph::getPin(ed::PinId pinId) {
    ...
    UiPinPtr nullPin = std::make_shared<UiPin>(-10000, "nullPin", "null", nullptr, ax::NodeEditor::PinKind::Output,
                                               nullptr, nullptr);
    return nullPin;
}
        ...
        if (node->getCategory() == "group") {
            ...
            if (node->getNode() != nullptr) {
                ...
                ImGui::GetWindowDrawList()->AddRectFilled(
                        ...
                        ImVec2(ed::GetNodeSize(node->getId()).x - 9.f, ImGui::GetTextLineHeight() + 2.f),
    ...
    if (inputPinId && outputPinId && (outputPin->_type == inputPin->_type)) {
        if (inputPin->_connected == false) {
            ...
            if (_graphNodes[downNode]->getInput() && _isNodeGraph) {
                ...
                mx::ShaderNodeImplPtr impl = shadergen.getImplementation(*_graphNodes[upNode]->getNode()->getNodeDef(),
                    ...
                    for (UiPinPtr pin: _graphNodes[downNode]->inputPins) {
                        if (pin->_pinId == outputPinId) {
                            ...
                            if (_graphNodes[downNode]->getNode() &&
                                _graphNodes[downNode]->getNode()->getType() == mx::SURFACE_SHADER_TYPE_STRING) {
                                    ...
                                    if (_graphNodes[upNode]->getNodeGraph() != nullptr) {
                                        for (UiPinPtr outPin: _graphNodes[upNode]->outputPins) {
                                            ...
                                            if (outPin->_pinId == inputPinId) {
                                                mx::OutputPtr outputs = _graphNodes[upNode]->getNodeGraph()->getOutput(
                                                        outPin->_name);
    ...
    if (_graphNodes[downNode]->getNode()) {
        ...
        for (UiPinPtr pin: _graphNodes[downNode]->inputPins) {
            if ((int) pin->_pinId.Get() == endAttr) {
                removeEdge(downNode, upNode, pin);
                if (_graphNodes[upNode]->getInput()) {
                    _graphNodes[downNode]->getNodeGraph()->getInput(pin->_name)->removeAttribute(
    ...
    if (node->outputPins.size() > 0) {
        ...
        for (UiPinPtr pin: node->outputPins.front()->getConnections()) {
            ...
            if (pin->_pinNode->getNode()) {
                ...
                if (node->getInput()) {
                    pin->_pinNode->getNodeGraph()->getInput(pin->_name)->removeAttribute(
                            mx::ValueElement::INTERFACE_NAME_ATTRIBUTE);
...
void Graph::propertyEditor() {
    ...
    if (_currUiNode) {
        ...
        ImGuiTableFlags tableFlags =
                ImGuiTableFlags_ScrollY | ImGuiTableFlags_Resizable | ImGuiTableFlags_NoSavedSettings |
        ...
        if (_currUiNode->getNode()) {
            ...
            for (UiPinPtr input: _currUiNode->inputPins) {
                if (_currUiNode->_showAllInputs ||
                    (input->getConnected() || _currUiNode->getNode()->getInput(input->_name))) {
                    count++;
                }
            }
            if (count) {
                ...
                if (haveTable) {
                    ...
                    for (UiPinPtr input: _currUiNode->inputPins) {
                        if (_currUiNode->_showAllInputs ||
                            ...
                            std::string inputLabel = !uiProperties.uiName.empty() ? uiProperties.uiName
                            ...
                            mx::InputPtr tempInt = _currUiNode->getNode()->getNodeDef()->getActiveInput(
                                    input->_input->getName());
                            docString += input->_name;
                            docString += ": ";
                            if (tempInt) {
                                std::string newStr = _currUiNode->getNode()->getNodeDef()->getActiveInput(
                                        input->_input->getName())->getDocString();
            ...
            for (UiPinPtr input: inputs) {
                if (_currUiNode->_showAllInputs ||
                    (input->getConnected() || _currUiNode->getNodeGraph()->getInput(input->_name))) {
                    count++;
                }
            }
            if (count) {
                ...
                if (haveTable) {
                    ...
                    for (UiPinPtr input: inputs) {
                        if (_currUiNode->_showAllInputs ||
                            ...
                            docString += _currUiNode->getNodeGraph()->getActiveInput(
                                    input->_input->getName())->getDocString();

                            ImGui::TableNextColumn();
                            if (!input->_input->getConnectedNode() &&
                                _currUiNode->getNodeGraph()->getActiveInput(input->_input->getName())) {
                                setConstant(_currUiNode, input->_input, uiProperties);
                            } else {
                                std::string typeText = " [" + input->_input->getType() + "]";
                                ImGui::Text("%s", typeText.c_str());
...
void Graph::addNodePopup(bool cursor) {
    bool open_AddPopup =
            ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && ImGui::IsKeyReleased(ImGuiKey_Tab);
    ...
    if (ImGui::BeginPopup("add node")) {
        ...
        for (std::unordered_map<std::string, std::vector<std::vector<std::string>>>::iterator it = _extraNodes.begin();
             it != _extraNodes.end(); ++it) {
            ...
            if (subs.size() > 0) {
                ...
                for (size_t i = 0; i < it->second.size(); i++) {
                    ...
                    if (str.find(subs) != std::string::npos) {
                        if (ImGui::MenuItem(getNodeDefId(nodeName).c_str()) ||
                ...
                if (ImGui::BeginMenu(it->first.c_str())) {
                    ...
                    for (size_t j = 0; j < it->second.size(); j++) {
                        ...
                        if (ImGui::MenuItem(getNodeDefId(name).c_str()) ||
                            (ImGui::IsItemFocused() && ImGui::IsKeyPressedMap(ImGuiKey_Enter))) {
        ...
        for (std::unordered_map<std::string, std::vector<mx::NodeDefPtr>>::iterator it = _nodesToAdd.begin();
             it != _nodesToAdd.end(); ++it) {
            // filter out list of nodes
            if (subs.size() > 0) {
                ImGui::SetNextWindowSizeConstraints(ImVec2(250.0f, 300.0f), ImVec2(-1.0f, 500.0f));
                for (size_t i = 0; i < it->second.size(); i++) {
...
void Graph::searchNodePopup(bool cursor) {
    const bool open_search =
            ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && ImGui::IsKeyDown(ImGuiKey_F) &&
...
void Graph::shaderPopup() {
    if (_renderer->getMaterialCompilation()) {
        ImGui::SetNextWindowPos(
                ImVec2((float) _renderer->getViewWidth() - 135, (float) _renderer->getViewHeight() + 5));
...
void Graph::handleRenderViewInputs(ImVec2 minValue, float width, float height) {
    ...
    if (mousePos.x > minValue.x && mousePos.x < (minValue.x + width) && mousePos.y > minValue.y &&
        mousePos.y < (minValue.y + height)) {
...
void Graph::drawGraph(ImVec2 mousePos) {
    ...
    ImGui::SetNextWindowPos(ImVec2(io2.DisplaySize.x * 0.5f, io2.DisplaySize.y * 0.5f), ImGuiCond_Always,
                            ImVec2(0.5f, 0.5f));
    ImGui::Begin("MaterialX", nullptr,
    ...
    ed::Begin("My Editor");
    {
        ...
        if (selectedNodes.size() > 0) {
            ...
            if (graphPos > -1) {
                ...
                if (!_prevUiNode || (_prevUiNode->getName() != _graphNodes[graphPos]->getName())) {
                    ...
                    if (_currUiNode->getNode()) {
                        if (_currUiNode->getNode()->getType() == mx::SURFACE_SHADER_TYPE_STRING ||
                            _currUiNode->getNode()->getType() == mx::MATERIAL_TYPE_STRING) {
        ...
        if (ImGui::IsKeyReleased(ImGuiKey_U) && (!ImGui::IsPopupOpen("add node")) && (!ImGui::IsPopupOpen("search")) &&
