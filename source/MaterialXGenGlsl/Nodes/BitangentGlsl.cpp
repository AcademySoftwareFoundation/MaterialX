...
GenImplementationPtr BitangentGlsl::create()
{
    return std::make_shared<BitangentGlsl>();
}

void BitangentGlsl::createVariables(const DagNode& node, ShaderGenerator& /*shadergen*/, Shader& shader_)
{
...
void BitangentGlsl::emitFunctionCall(const DagNode& node, GenContext& context, ShaderGenerator& shadergen, Shader& shader_)
{
    HwShader& shader = static_cast<HwShader&>(shader_);
