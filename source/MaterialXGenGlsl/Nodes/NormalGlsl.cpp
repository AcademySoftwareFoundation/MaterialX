...
ShaderImplementationPtr NormalGlsl::create()
{
    return std::make_shared<NormalGlsl>();
}

void NormalGlsl::createVariables(const ShaderNode& node, ShaderGenerator& /*shadergen*/, Shader& shader_)
{
...
void NormalGlsl::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderGenerator& shadergen, Shader& shader_)
{
    HwShader& shader = static_cast<HwShader&>(shader_);
