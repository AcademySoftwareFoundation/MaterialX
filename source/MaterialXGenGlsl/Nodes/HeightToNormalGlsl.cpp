...
GenImplementationPtr HeightToNormalGlsl::create()
{
...
void HeightToNormalGlsl::emitFunctionCall(const DagNode& node, GenContext& context, ShaderGenerator& shadergen, Shader& shader_)
{
    const DagInput* inInput = node.getInput("in");
    const DagInput* scaleInput = node.getInput("scale");
