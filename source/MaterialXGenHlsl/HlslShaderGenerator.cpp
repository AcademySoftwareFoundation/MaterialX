...
// Recursively rewrite floatN(<single-arg-expr>) into (floatN)(<single-arg-expr>).
// Multi-argument ctors are kept as-is structurally, but their bodies are
// processed recursively so nested single-arg ctors don't escape (the
// luminance_color4 case: float4(float3(dot(...)), a)).
std::string rewriteHlslSplatsRange(std::string::const_iterator first,
                                   std::string::const_iterator last)
{
    static const std::regex prefix(R"(\bfloat([234])\s*\()");
    std::string out;
    out.reserve(static_cast<std::size_t>(last - first) + 32);

    auto cur = first;
    std::smatch m;
    while (std::regex_search(cur, last, m, prefix))
    {
        out.append(cur, m[0].first);

        const char width = *m[1].first;
        auto inner = m[0].second; // just past the opening '('

        int depth = 1;
        bool hasTopLevelComma = false;
        auto scan = inner;
        while (scan != last && depth > 0)
        {
            const char c = *scan;
            if (c == '(')
            {
        ...
        if (scan == last)
        {
            // Unbalanced parens; emit the rest verbatim and stop.
            out.append(m[0].first, last);
            cur = last;
            break;
        }

        std::string innerRewritten = rewriteHlslSplatsRange(inner, scan);
        if (hasTopLevelComma)
        {
            // Multi-argument ctor - keep the floatN(...) shape but use the
            // rewritten body so any nested single-arg ctors are corrected.
            out += "float";
            out += width;
            out += '(';
            out += innerRewritten;
        ...
        {
            // Single-argument ctor - rewrite to (floatN)(...) cast splat.
            out += '(';
            out += "float";
            out += width;
            out += ")(";
...
std::string rewriteHlslSplats(const std::string& src)
{
    return rewriteHlslSplatsRange(src.cbegin(), src.cend());
}

} // namespace

const string HlslShaderGenerator::TARGET = "genhlsl";
const string HlslShaderGenerator::VERSION = "6.0";
...
ShaderPtr HlslShaderGenerator::generate(const string& name, ElementPtr element, GenContext& context) const
{
    ...
    {
        // All semantics that, in HLSL, can have an index first check for `name_` and only then for `name`.
        // HwShaderGenerator generates index variables as `name_X` and HLSL needs `SEMANTICX`.
        ...
        // This replaces tokens without assuming that they are just alphanumeric, so we can turn
        // HW::T_IN_TEXCOORD + "_" into `TEXCOORD` because tokenSubstitution does not work on it.
        ...
        // If the input has a special semantic (e.g., POSITION[n]) then it is generated from the variable name.
        // Otherwise, the variable name itself is used as a semantic.
...
void HlslShaderGenerator::emitVertexStage(const ShaderGraph& graph, GenContext& context, ShaderStage& stage) const
{
    ...
    // Add main function
    setFunctionName("VSMain", stage);
    emitLine(vertexData.getName() + " VSMain(" + vertexInputs.getName() + " vi)", stage, false);
    emitFunctionBodyBegin(graph, context, stage);

    // Assign input variables from the `vi` input struct to static variables to mimic the GLSL globals.
...
void HlslShaderGenerator::emitDirectives(GenContext&, ShaderStage&) const
{
}
...
void HlslShaderGenerator::emitUniforms(GenContext& context, ShaderStage& stage, bool emitLighting) const
{
    HwResourceBindingContextPtr bindingCtx = getResourceBindingContext(context);

    // Emit MAX_LIGHT_SOURCES and the LightData struct definition only once.
    // When a binding context is attached its emitStructuredResourceBindings()
    // is responsible for wrapping the struct array in a cbuffer; it would
    // re-emit the struct itself, so we skip the inline struct emission here
    ...
    if (bindingCtx)
    {
        bindingCtx->initialize();
        bindingCtx->emitDirectives(context, stage);
        ...
        for (const auto& it : stage.getUniformBlocks())
        {
            ...
            if (!uniforms.empty() && uniforms.getName() != HW::LIGHT_DATA)
            {
                ...
                bindingCtx->emitResourceBindings(context, uniforms, stage);
            }
        }
        ...
        {
            const VariableBlock& lightData = stage.getUniformBlock(HW::LIGHT_DATA);
            const string arraySuffix = "[" + HW::LIGHT_DATA_MAX_LIGHT_SOURCES + "]";
            bindingCtx->emitStructuredResourceBindings(context, lightData, stage,
                                                      lightData.getInstance(), arraySuffix);
            emitLineBreak(stage);
        }
    ...
    // Default path (no explicit binding context). HLSL forbids texture or
    // sampler members inside a cbuffer, so we partition each block: value
    // uniforms go into a single cbuffer per stage, and FILENAME uniforms
    // (SamplerTexture2D handles) are emitted as separate file-scope
    // declarations below the cbuffer.
    bool hasAnyValueUniform = false;
    for (const auto& it : stage.getUniformBlocks())
    {
        const VariableBlock& uniforms = *it.second;
        if (uniforms.empty() || uniforms.getName() == HW::LIGHT_DATA)
        ...
        for (auto u : uniforms.getVariableOrder())
        {
            ...
            {
                hasAnyValueUniform = true;
                break;
            }
        }
        if (hasAnyValueUniform)
        {
            break;
        }
    }

    if (hasAnyValueUniform || emitLighting)
    {
        ...
        for (const auto& it : stage.getUniformBlocks())
        {
            ...
            bool blockHasValueUniform = false;
            for (auto u : uniforms.getVariableOrder())
            {
                if (u->getType() != Type::FILENAME)
                {
                    blockHasValueUniform = true;
                    break;
                }
            }
            if (!blockHasValueUniform)
            {
                continue;
            }
            ...
            for (auto u : uniforms.getVariableOrder())
            {
                ...
                emitLineBegin(stage);
                emitVariableDeclaration(u, _syntax->getUniformQualifier(), context, stage, false);
        ...
        if (emitLighting)
        {
            const VariableBlock& lightData = stage.getUniformBlock(HW::LIGHT_DATA);
            const string arraySuffix = "[" + HW::LIGHT_DATA_MAX_LIGHT_SOURCES + "]";
            emitLine("uniform " + lightData.getName() + " " + lightData.getInstance() + arraySuffix, stage);
        }
    ...
    // FILENAME uniforms (SamplerTexture2D) at file scope.
    for (const auto& it : stage.getUniformBlocks())
    {
        const VariableBlock& uniforms = *it.second;
        if (uniforms.empty() || uniforms.getName() == HW::LIGHT_DATA)
...
void HlslShaderGenerator::emitPixelStage(const ShaderGraph& graph, GenContext& context, ShaderStage& stage) const
{
    ...
    // Add textures
    emitLibraryInclude("stdlib/genhlsl/lib/mx_texture.hlsl", context, stage);
    ...
    // Add main function
    setFunctionName("PSMain", stage);
    emitLine("float4 PSMain(" + vertexData.getName() + " _vd) : SV_Target", stage, false);
    emitFunctionBodyBegin(graph, context, stage);
    ...
    // It is a variable local to PSMain, and returned at the end of it.
    emitOutputs(context, stage);
    ...
    else
    {
        ...
        if (outputConnection)
        {
            if (graph.hasClassification(ShaderNode::Classification::SURFACE))
            {
                ...
                {
                    emitLine("float outAlpha = clamp(1.0 - dot(" + outTransparency + ", float3(0.3333, 0.3333, 0.3333)), 0.0, 1.0)", stage);
...
void HlslShaderGenerator::toVec4(TypeDesc type, string& variable)
{
    if (type.isFloat3())
    {
...
void HlslShaderGenerator::HlslSyntaxFromGlsl(ShaderStage& shaderStage) const
{
    std::string sourceCode = shaderStage.getSourceCode();

    // Renames GLSL constructs that are used in shared code to HLSL equivalent constructs.
    std::unordered_map<string, string> replaceTokens;
    replaceTokens["sampler2D"] = "SamplerTexture2D";
    // Map GLSL derivative intrinsics to namespaced HLSL wrappers (defined
    // in stdlib/genhlsl/lib/mx_math.hlsl). We deliberately do NOT use the
    // bare HLSL names ddx/ddy here: shared GLSL library code (mx_hextile)
    // declares local variables named `ddx` and `ddy`, and renaming our
    // token to the HLSL intrinsic would make the local-variable
    // declaration shadow the function on its own RHS.
    replaceTokens["dFdy"] = "mx_ddy";
    replaceTokens["dFdx"] = "mx_ddx";
    ...
    size_t pos = 0;
    // Standard identifier-boundary semantics: a token is replaced only when
    // the characters immediately before and after are NOT identifier
    // continuation chars (letter, digit, underscore). The previous narrow
    // allow-list missed common operators like `;`, `*`, `=`, `.` - that
    // caused e.g. `- mix;` in lama_mix to escape the `mix` -> `lerp`
    // rename while the function signature `float mix,` was caught,
    // leaving an undeclared identifier in the body.
    ...
    // The bare identifier "texture" is a reserved type name in HLSL (the
    // legacy D3DX Effects framework type), so a function call texture(s, uv)
    // is a syntax error even if the user-defined function is in scope.
    // Rewrite the GLSL-style sampler call into our renamed helper. We
    // intentionally do not match textureLod / textureGrad / textureSize -
    // those names are not reserved in HLSL and our helpers keep them as-is.
    {
        static const std::regex textureCallRe(R"(\btexture\s*\()");
        sourceCode = std::regex_replace(sourceCode, textureCallRe, std::string("mx_texture_sample("));
    }

    // DXC under HLSL 2021 rejects single-argument vector constructors that GLSL
    // (and Slang) accept as scalar splats. The argument can be a literal
    // (float3(0.0)), an identifier (float3(coat_gamma_out)), an arithmetic
    // expression (float3(1.0 - dirAlbedo * weight)), or a function call
    // (float3(dot(_in, lumacoeffs))) - all need to become a cast splat.
    //
    // Rewrite every floatN(<single-arg-expr>) to (floatN)(<single-arg-expr>).
    // The cast broadcasts a scalar to all components, is a no-op when the
    // operand is already a vector of the right width, and evaluates the
    // operand exactly once. "Single argument" means no comma at the top level
    // of the parenthesized expression - commas inside nested calls don't count.
    //
    // The scanner recurses into multi-argument constructor bodies so that
    // single-arg ctors nested inside multi-arg ones - e.g. the inner
    // float3(dot(...)) inside float4(float3(dot(...)), a) emitted by
    ...
    // "return ClosureData(closureType, L, V, N, P, occlusion);" which compiles
    // in GLSL/MSL/Slang but not in HLSL. Rewrite it into a temporary
    // declaration plus return, which is the form DXC accepts.
    static const std::regex returnCtorRe(R"(return\s+ClosureData\s*\(([^)]*)\)\s*;)");
    sourceCode = std::regex_replace(sourceCode, returnCtorRe,
                                    std::string("ClosureData _hlsl_cd = {$1}; return _hlsl_cd;"));
