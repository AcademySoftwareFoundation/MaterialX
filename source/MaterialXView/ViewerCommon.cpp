...
{
#if MATERIALXVIEW_METAL_BACKEND
    #define VS_POST_FIX "_vs.metal"
    #define PS_POST_FIX "_ps.metal"
#else
    #define VS_POST_FIX "_vs.glsl"
    #define PS_POST_FIX "_ps.glsl"
#endif
    try
    {
        ...
        if (elem)
        {
            ...
            mx::FilePath pixelSourceFilename = sourceFilename.asString() + PS_POST_FIX;
            mx::FilePath vertexSourceFilename = sourceFilename.asString() + VS_POST_FIX;
...
#ifdef MATERIALXVIEW_METAL_BACKEND
#define MATERIAL_TYPE     mx::MslMaterial
#define MATERIAL_TYPE_PTR mx::MslMaterialPtr
#else
#define MATERIAL_TYPE     mx::GlslMaterial
#define MATERIAL_TYPE_PTR mx::GlslMaterialPtr
#endif
        ...
        for (size_t i=0; i<renderablePaths.size(); i++)
        {
            ...
            if (udimSetValue && udimSetValue->isA<mx::StringVec>())
            {
                ...
                {
                    mx::MaterialPtr mat = MATERIAL_TYPE::create();
                    mat->setDocument(doc);
        ...
        if (!newMaterials.empty())
        {
            ...
            MATERIAL_TYPE_PTR udimMaterial = nullptr;
            for (mx::MaterialPtr m : newMaterials)
            {
                MATERIAL_TYPE_PTR mat = std::static_pointer_cast<MATERIAL_TYPE>(m);
