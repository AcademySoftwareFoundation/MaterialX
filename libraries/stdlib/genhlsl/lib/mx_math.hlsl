...
// translate dFdx / dFdy to a #define alias of the HLSL intrinsic - the
// macro would expand inline, and the local variable on the LHS would
// shadow the intrinsic on the RHS. Real functions sidestep that because
// `mx_ddx` survives as a function symbol independent of any local `ddx`.
float  mx_ddx(float  p) { return ddx(p); }
float2 mx_ddx(float2 p) { return ddx(p); }
float3 mx_ddx(float3 p) { return ddx(p); }
float4 mx_ddx(float4 p) { return ddx(p); }
float  mx_ddy(float  p) { return ddy(p); }
float2 mx_ddy(float2 p) { return ddy(p); }
...
{
    bool3 isAbove = (color > float3(0.0031308, 0.0031308, 0.0031308));
    float3 linSeg = color * 12.92;
    float3 powSeg = 1.055 * pow(max(color, float3(0.0, 0.0, 0.0)), float3(1.0 / 2.4, 1.0 / 2.4, 1.0 / 2.4)) - 0.055;
    // HLSL has no per-component select() prior to SM 6.7; use a per-component ternary.
    return float3(isAbove.x ? powSeg.x : linSeg.x,
                  isAbove.y ? powSeg.y : linSeg.y,
                  isAbove.z ? powSeg.z : linSeg.z);
}
